...
export function createIssuesSystem({ db, publicOrigin, json, bodyJson }) {
  ...
  function publicAuthor(userId) { return `用户 #${Number(userId)}`; }
  function issueRow(row, admin = false) { return { id:row.id, title:row.title, body:row.body, status:row.status, pinned:Boolean(row.pinned), replyCount:Number(row.reply_count || 0), createdAt:row.created_at, updatedAt:row.updated_at, author: admin ? { id:row.user_id, email:row.email || '', label:publicAuthor(row.user_id) } : { label:publicAuthor(row.user_id) } }; }
  function replyRow(row, admin = false) { const isAdmin = row.author_role === 'admin'; return { id:row.id, body:row.body, authorRole:row.author_role, author:isAdmin ? { label:'GPTWork 管理员' } : (admin ? { id:row.user_id, email:row.email || '', label:publicAuthor(row.user_id) } : { label:publicAuthor(row.user_id) }), createdAt:row.created_at, updatedAt:row.updated_at }; }
  function getIssue(id, admin = false) { const row = db.prepare(`SELECT i.*,u.email,(SELECT COUNT(*) FROM support_issue_replies r WHERE r.issue_id=i.id) AS reply_count FROM support_issues i JOIN users u ON u.id=i.user_id WHERE i.id=?`).get(id); return row ? issueRow(row, admin) : null; }
  ...
  function rateCheck(kind, userId, max, minutes) { const since = new Date(Date.now() - minutes * 60000).toISOString(); const row = kind === 'issue' ? db.prepare('SELECT COUNT(*) AS count FROM support_issues WHERE user_id=? AND created_at>=?').get(userId,since) : db.prepare("SELECT COUNT(*) AS count FROM support_issue_replies WHERE user_id=? AND author_role='user' AND created_at>=?").get(userId,since); if (Number(row?.count || 0) >= max) fail(429,'RATE_LIMITED','提交过于频繁，请稍后再试'); }
  ...
  async function handleSite(req,res,url) {
    ...
    try {
      ...
      if (url.pathname === '/site/api/issues' && req.method === 'GET') {
        ...
        const rows=db.prepare(`SELECT i.*,u.email,(SELECT COUNT(*) FROM support_issue_replies r WHERE r.issue_id=i.id) AS reply_count FROM support_issues i JOIN users u ON u.id=i.user_id ${clause} ORDER BY i.pinned DESC,i.updated_at DESC,i.id DESC LIMIT ? OFFSET ?`).all(...params,cfg.pageSize,(page-1)*cfg.pageSize).map((row)=>issueRow(row));
      ...
      if (url.pathname === '/site/api/issues' && req.method === 'POST') {
        if(!cfg.createEnabled) fail(403,'CREATE_DISABLED','当前暂停新建问题'); const user=requireUser(req); rateCheck('issue',user.user_id,5,10); const input=await bodyJson(req); const title=cleanTitle(input.title,cfg.maxTitle); const body=cleanBody(input.body,cfg.maxBody);
        if(title.length<4) fail(400,'TITLE_TOO_SHORT','标题至少 4 个字符'); if(body.length<10) fail(400,'BODY_TOO_SHORT','问题描述至少 10 个字符'); const now=nowIso(); const result=db.prepare('INSERT INTO support_issues(user_id,title,body,status,pinned,created_at,updated_at) VALUES(?,?,?,?,?,?,?)').run(user.user_id,title,body,'open',0,now,now);
  ...
  async function handleAdmin(req,res,url) {
    ...
    try {
      ...
      if(detail&&req.method==='PATCH'){const id=Number(detail[1]);const current=getIssue(id,true);if(!current)fail(404,'ISSUE_NOT_FOUND','问题不存在');const input=await bodyJson(req);const cfg=config();const status=['open','closed'].includes(input.status)?input.status:current.status;const pinned=input.pinned===undefined?current.pinned:Boolean(input.pinned);const title=input.title===undefined?current.title:cleanTitle(input.title,cfg.maxTitle);const body=input.body===undefined?current.body:cleanBody(input.body,cfg.maxBody);if(title.length<4||body.length<10)fail(400,'INVALID_CONTENT','标题或正文过短');db.prepare('UPDATE support_issues SET title=?,body=?,status=?,pinned=?,updated_at=? WHERE id=?').run(title,body,status,pinned?1:0,nowIso(),id);json(res,200,{ok:true,issue:getIssue(id,true),replies:getReplies(id,true)});return true;}
