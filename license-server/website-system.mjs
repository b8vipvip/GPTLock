  ...
  homeModules: [
    {
      ...
      badge: 'Windows / Linux · Chrome / Edge',
      title: 'GPTWork\n能做什么？',
    ...
    {
      id: 'features', type: 'features', name: '核心功能', enabled: true, order: 20,
      title: '围绕两件事：\nWork 模式与模型锁定。',
      lead: '把常用设置固定下来，并持续确认实际状态；出现不符合锁定策略的情况时给出明确提示。',
      items: [
    ...
    {
      id: 'workflow', type: 'workflow', name: '使用流程', enabled: true, order: 30,
      title: '从安装到日常使用，\n只保留必要步骤。',
      lead: '安装完成后，登录账户、选择偏好并开启 GPTWork。之后正常使用 ChatGPT；只有遇到异常时才需要诊断或更新。',
      items: [
        { title: '① 安装', body: '安装正式版本并启用扩展' },
        { title: '② 登录', body: '使用 GPTWork 账户' },
        { title: '③ 设置', body: '选择 Work 模式、模型与推理偏好' },
        { title: '④ 开启', body: '确认锁定与验证状态正常' },
        { title: '⑤ 聊天', body: '像平时一样使用 ChatGPT' },
        { title: '⑥ 维护', body: '需要时诊断、更新或管理账户' },
      ],
      primaryLabel: '查看使用教程', primaryHref: '/guide', secondaryLabel: '进入 Issues 讨论区', secondaryHref: '/issues',
    },
    {
      id: 'callout', type: 'callout', name: '底部引导', enabled: true, order: 40,
      title: '下载、教程、账户、Issues，一个网站就够。',
      body: '正式版本、使用教程、账户管理和公开问题讨论都集中在 GPTWork 官网。',
      buttonLabel: '打开 Issues', buttonHref: '/issues',
    },
  ],
};
...
function order(value, fallback) { const parsed = Number(value); return Number.isFinite(parsed) ? Math.max(-9999, Math.min(9999, Math.round(parsed))) : fallback; }
function safeHref(value, fallback = '/') {
  const href = text(value, fallback, 500);
  if (/^\/(?!\/)[^\s]*$/.test(href)) return href;
  try { const url = new URL(href); if (url.protocol === 'https:') return url.toString(); } catch {}
  return fallback;
}
function slug(value, fallback) {
  const result = String(value || '').toLowerCase().replace(/[^a-z0-9_-]/g, '').slice(0, 48);
  return result || fallback;
}
function normalizeItems(value, defaults, maxItems = 8) {
  const source = Array.isArray(value) ? value : defaults;
  return source.slice(0, maxItems).map((item, index) => ({
    title: text(item?.title, defaults[index]?.title || `项目 ${index + 1}`, 160),
    body: text(item?.body, defaults[index]?.body || '', 1200),
  }));
}

function normalizeModule(raw, fallback, index) {
...
export function normalizeWebsiteConfig(input = {}) {
  const defaults = DEFAULT_WEBSITE_CONFIG;
  const site = input.site || {};
  const configuredNavigation = Array.isArray(input.navigation) ? input.navigation : defaults.navigation;
  const navigation = configuredNavigation.slice(0, 16).map((item, index) => {
    const fallback = defaults.navigation.find((candidate) => candidate.id === item?.id) || defaults.navigation[index] || { id: `nav-${index + 1}`, label: '链接', href: '/', enabled: true, order: (index + 1) * 10 };
    return { id: slug(item?.id, fallback.id), label: text(item?.label, fallback.label, 80), href: safeHref(item?.href, fallback.href), enabled: bool(item?.enabled, fallback.enabled), order: order(item?.order, fallback.order), account: bool(item?.account, fallback.account || false) };
  });
  const sourceModules = Array.isArray(input.homeModules) ? input.homeModules : defaults.homeModules;
  const modules = [];
  const seen = new Set();
  for (let index = 0; index < Math.min(sourceModules.length, 20); index += 1) {
    const raw = sourceModules[index] || {};
    const fallback = defaults.homeModules.find((module) => module.id === raw.id) || (!raw.type || raw.type !== 'custom' ? defaults.homeModules[index] : null);
    const normalized = normalizeModule(raw, fallback, index);
    if (seen.has(normalized.id)) normalized.id = `custom-${index + 1}`;
    seen.add(normalized.id); modules.push(normalized);
  }
  for (const defaultModule of defaults.homeModules) {
    if (!seen.has(defaultModule.id)) modules.push(clone(defaultModule));
  }
  return {
    schemaVersion: 1,
    site: {
      brandName: text(site.brandName, defaults.site.brandName, 80),
      title: text(site.title, defaults.site.title, 160),
      description: text(site.description, defaults.site.description, 500),
      footerText: text(site.footerText, defaults.site.footerText, 160),
    },
    navigation,
    homeModules: modules,
  };
...
export function createWebsiteSystem({ db, json }) {
  ...
  function read() {
    const row = getSetting.get(WEBSITE_SETTING_KEY);
    if (!row) return { config: clone(DEFAULT_WEBSITE_CONFIG), updatedAt: null };
    try { return { config: normalizeWebsiteConfig(JSON.parse(row.value)), updatedAt: row.updated_at || null }; }
    catch { return { config: clone(DEFAULT_WEBSITE_CONFIG), updatedAt: row.updated_at || null }; }
  }
  function save(config, event = 'website_config_updated') {
    const normalized = normalizeWebsiteConfig(config);
    const updatedAt = new Date().toISOString();
    setSetting.run(WEBSITE_SETTING_KEY, JSON.stringify(normalized), updatedAt);
    audit.run(event, JSON.stringify({ modules: normalized.homeModules.length, navigation: normalized.navigation.length }), updatedAt);
    return { config: normalized, updatedAt };
  }
  ...
  async function handleAdmin(req, res, url, bodyJson) {
    if (url.pathname === '/admin/api/website' && req.method === 'GET') { json(res, 200, { ok: true, ...read(), defaults: clone(DEFAULT_WEBSITE_CONFIG) }); return true; }
