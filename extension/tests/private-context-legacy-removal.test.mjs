...
test('manifest loads private send-budget authority plus the public verified chat-length estimator', () => {
  ...
  assert.match(chatLengthView, /estimateTextTokens/);
  assert.match(chatLengthView, /computeLocalBudget/);
  assert.match(chatLengthView, /learned-chatgpt-thread-boundary/);
  assert.doesNotMatch(chatLengthView, /__GPTLOCK_PRIVATE_CONTEXT_BUDGET_AUTHORITY__/);
});
