...
GPTWork 的公开安全文档只描述用户需要知道的边界，不公开可用于复刻核心实现的检测规则、字段清单、判定权重、内部状态机或通信细节。

## 用户数据原则

- GPTWork 只应处理完成产品功能所需要的数据；
...
GPTWork 运行在用户自己的浏览器和设备上，因此不能把客户端侧机制描述成不可破解的 DRM，也不能承诺绕过 ChatGPT 的套餐、额度、区域或模型可用性限制。

GPTWork 的状态和诊断用于帮助用户了解产品是否按预期工作，但不应把客户端观察结果宣传成对第三方后端内部调度的密码学证明。
