...
- **正常/已开启**：可以按当前设置继续使用；
- **未确认/等待**：当前信息不足以给出更强确认，不等于聊天失败；
- **警告**：某个组件或页面状态与预期不同，可先刷新或运行自动验证；
- **异常**：按界面提示修复、重连、重新登录或更新版本。

具体内部判断过程不属于用户操作要求，也不会在公开文档中展开。
...
访问 `https://gptlock.mv3.cn/account` 可以管理自己的权益、设备、会话和密码。

## 保持更新

建议始终使用正式发布版本。下载页：`https://gptlock.mv3.cn/releases`。
