...
function replaceOnce(source, from, to, label) {
  ...
  if (source.indexOf(from, index + from.length) >= 0) throw new Error(`Patch anchor is not unique: ${label}`);
...
// 1) Learn from ChatGPT's real, visible conversation-length boundary without pretending it is an official model-window token counter.
{
  const path = 'extension/context-budget.js';
...
`  function computeBudget({
  ...
  function conversationMessageText`, 'compute budget with learned hard upper bound');

  source = replaceRegex(source,
/  function nextLearnedProfile\(\{[\s\S]*?\n  \}\n\n  const api = \{/,
`  function nextLearnedProfile({
  ...
  const api = {`, 'learned profile + hard limit helpers');
...
`  function visible(element) {
  ...
  function findComposer() {
`, 'visible hard-limit detector');

  source = replaceRegex(source,
/  function activeAdaptiveLimit\(model\) \{[\s\S]*?\n  \}\n\n  function snapshotNow\(\) \{/,
`  function activeAdaptiveLimit(model) {
  ...
  function snapshotNow() {`, 'active hard limit profile');
...
`, 'snapshot applies hard upper bound');
...
`, 'recompute learns hard-limit notice');
...
`, 'hard limit is a failed generation for bypass learning');
...
`  async function persistHardLimitObservation(snapshot, notice) {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('gptlock:context-hard-limit-learned', { detail }));
...
`    conversationMetricsCheckedAt = 0;
    ...
    scheduleRefresh();
`, 'reset hard-limit notice on navigation');

  await save(path, source);
}

// 2) Keep a backward-compatible bridge for a stale pre-account popup so a partially reloaded browser never blocks on removed license messages.
{
...
`      case 'GPTLOCK-LICENSE-GET':
      ...
      case 'GPTLOCK_ACCOUNT_CONFIG':
        return accountClient.config();
`, 'legacy popup compatibility bridge');
  await save(path, source);
}

// 3) Server-side admin: editable email/status/effective expiry/limits plus secure administrator password reset.
{
  const path = 'license-server/account-system.mjs';
  let source = await text(path);
  source = replaceRegex(source,
/      if \(userMatch && req\.method === 'PATCH'\) \{[\s\S]*?\n      \}\n\n      const resetDevicesMatch/,
`      if (userMatch && req.method === 'PATCH') {
      ...
      const resetDevicesMatch`, 'admin user patch and password route');
  await save(path, source);
}

// 4) Admin page: inline user/entitlement editors and a centered password dialog; no browser prompt/alert flow for user edits.
{
  const path = 'license-server/public/admin.html';
  let source = await text(path);
  source = replaceOnce(source,
...
{
  const path = 'license-server/public/admin.css';
  let source = await text(path);
...
{
  const path = 'license-server/public/admin.js';
  let source = await text(path);
  source = replaceOnce(source,
...
`, 'admin password element refs');

  source = replaceRegex(source,
/function promptNumber\([\s\S]*?\n\}\n\nfunction renderDashboard/,
...
function renderDashboard`, 'replace prompt helper');

  source = replaceRegex(source,
/async function editUser\(row\) \{[\s\S]*?\n\}\n\nasync function toggleUser\(row\) \{[\s\S]*?\n\}\n\nasync function grantMembership\(row\) \{[\s\S]*?\n\}\n\nasync function resetDevices\(row\) \{[\s\S]*?\n\}\n\nfunction renderUsers\(rows\) \{[\s\S]*?\n\}\n\nasync function loadUsers/,
...
async function loadUsers`, 'inline user editor and password modal logic');
...
el.refreshOrders.addEventListener('click', () => void loadOrders());
`, 'password dialog event bindings');
  await save(path, source);
}

// 5) Regression tests for the real limit text, conservative upper-bound learning, stale license popup, and admin password reset.
{
  const path = 'extension/tests/context-budget.test.mjs';
  let source = await text(path);
...
{
  const path = 'extension/tests/license-routing.test.mjs';
  let source = await text(path);
  source = source.replace("test('legacy license listener is removed and account messages are handled by background'", "test('legacy license UI is removed while background keeps a stale-popup migration bridge'");
  source = replaceOnce(source,
...
`, 'license migration regression assertions');
  await save(path, source);
}

{
  const path = 'license-server/test/smoke.test.mjs';
  let source = await text(path);
  source = replaceOnce(source,
...
`, 'admin UI regression assertions');
...
`    assert.equal(weakManual.data.error.code, 'WEAK_PASSWORD');
    ...
    // Inline admin PATCH can change email/status/effective expiry and custom limits without a browser prompt flow.
    ...
    // Admin can also explicitly create a verification-exempt account even while global verification is enabled.
`, 'admin password + inline edit smoke test');
  await save(path, source);
}

// 6) Publish as v0.5.11 so Windows/Linux users can actually receive the fix set.
for (const path of ['extension/manifest.json', 'extension/package.json', 'native-core/Cargo.toml', 'native-core/Cargo.lock']) {
  let source = await text(path);
  source = source.replace(/0\.5\.10/g, '0.5.11');
  await save(path, source);
}

console.log('Applied requested GPTLock fixes and bumped v0.5.11.');
